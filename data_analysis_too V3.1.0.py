...
def get_color(col):
    max, min = col.max(skipna = True), col.min(skipna = True)
...
class MyWindow(QWidget):
    ...
    def init_generate_plot_ui(self):
        ...
        for i, col in enumerate(numeric_df.columns):
            if pd.api.types.is_numeric_dtype(numeric_df[col]):
                ...
                def action(_=None, c=col):
                    ...
                    color_tip_label.setText(f"颜色标尺：{self.color_selected}")

                btn_color.clicked.connect(action)
                row = i // cols_per_row * 2 + 1
                col_pos = i % cols_per_row
                color_grid_layout.addWidget(btn_color, row + 1, col_pos)
        ...
        right_preview_layout.addWidget(color_tip_label)
        right_preview_layout.addLayout(color_grid_layout)
                    ...
                    for j, x_col in enumerate(self.selected_x):
                        ...
                        if kind == "line":
                            ...
                            if self.color_selected is not None:
                                colors = get_color(self.app.df[self.color_selected])
                                ax.scatter(self.app.df[x_col], self.app.df[y_col], c = colors, s=20)
                            else:
