...
class PlotPage(QWidget):
    ...
    def _setup_ui(self):
        ...
        btn_generate_box = QPushButton("生成箱线图")
        btn_generate_box.clicked.connect(lambda: self._generate_plot("box"))

        btn_generate_heatmap = QPushButton("生成热力图")
        btn_generate_heatmap.clicked.connect(lambda: self._generate_plot("heatmap"))
        ...
        left_layout.addWidget(btn_generate_box)
        left_layout.addWidget(btn_generate_heatmap)
    ...
    def _generate_boxplot(self):
        ...
        try:
            ...
            if not self.selected_y:
                ...
                data_to_plot = [df[col].dropna() for col in self.selected_y]
                labels = self.selected_y
            ...
            QMessageBox.critical(self, "错误", f"生成箱线图时出错：\n{str(e)}")
    ...
    def _generate_heatmap(self):
        ...
        try:
            ...
            if len(numeric_cols) < 2:
                QMessageBox.warning(self, "警告", "至少需要2个数值列才能生成热力图！")
                return

            # 如果有选择的列，使用选择的列；否则使用所有数值列
            selected_cols = []
            if self.selected_x:
                selected_cols.extend(self.selected_x)
            if self.selected_y:
                selected_cols.extend(self.selected_y)
            ...
            if not selected_cols:
                ...
                if len(selected_cols) < 2:
                    QMessageBox.warning(self, "警告", "选择的列中至少需要2个数值列！")
                    return
            ...
            QMessageBox.critical(self, "错误", f"生成热力图时出错：\n{str(e)}")
